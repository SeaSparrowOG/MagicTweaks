...
namespace Hooks::Tweaks::Archetypes
{
	bool ScriptEffect::Install() {
		...
		REL::Relocation<std::uintptr_t> VTABLE{ RE::Offset::ScriptEffect::VTABLE_ScriptEffect };
		_func = VTABLE.write_vfunc(offset, Thunk);
		logger::info("    >Installed Script VFunc hook"sv);
		return true;
	}

	void ScriptEffect::Thunk(RE::ActiveEffect* a_this,
		float a_delta) {
