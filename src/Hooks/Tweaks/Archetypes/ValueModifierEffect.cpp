...
namespace Hooks::Tweaks::Archetypes
{
	bool ValueEffect::Install() {
		...
		REL::Relocation<std::uintptr_t> VTABLE{ RE::Offset::ValueModifierEffect::VTABLE_ValueModifierEffect };
		_func = VTABLE.write_vfunc(offset, Thunk);
		logger::info("    >Installed Value Modifier VFunc hook"sv);
		return true;
	}

	void ValueEffect::Thunk(RE::ActiveEffect* a_this,
		float a_delta) {
